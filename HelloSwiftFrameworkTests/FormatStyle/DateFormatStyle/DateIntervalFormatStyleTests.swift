...
struct DateIntervalFormatStyleTests {
    ...
    @Test func testPredefinedStyles2() throws {
        var calendar = Calendar(identifier: .gregorian)
        calendar.locale = Locale(identifier: "ko_KR")
        calendar.timeZone = TimeZone(identifier: "Asia/Seoul")!
    ...
    @Test func testFactoryVariable() throws {
        ...
        let string1 = next30days.formatted(.interval.month(.wide).day())
        let string2 = next30days.formatted(Date.IntervalFormatStyle().month(.wide).day())

        #expect(string1 == string2)
        #expect(string1 == "10월 24일 ~ 11월 23일")
    }
